...
from my_profile_feed.models import user_profile, Notification
...
def PostListView(request):
    if request.session.has_key('username'):
        ...
        user_name = signup_model.objects.get(email=username).full_name



        recommendation=client.send(RecommendItemsToUser(username, 5))
        ...
        for c in recomm_post_id_list:
            # c = collection.find_one({"_id": ObjectId(str(p))})
        ...
        return render(request,'index.html',{'user_img':user_img,'postlist':postlist, 'profiles':profiles, 'all_profiles':all_profiles, 'current_user_followings':current_user_followings, 'follower':nfollower,'following':nfollowing, 'username':username, 'user_name':user_name, 'notifications':notifications, 'chats': chats, 'rec_profiles_list':rec_profiles_list})
...
def deletePostView(request):
    if request.session.has_key('username'):
        ...
        client.send(DeleteItem(c.get("_id")))
