    ...
    if request.session.has_key('username'):
        ...
        for c in chats_list:
            if chats_count < 5:
                ...
                break
        return render(request, "profiles.html", {'profiles':profiles, 'username':username, 'current_user_followings':current_user_followings, 'notifications':notifications, 'chats':chats})
    else:
    ...
    if request.session.has_key('username'):
        ...
        for c in chats_list:
            if chats_count < 5:
                ...
                break
        return render(request, "following.html", {'profiles':profiles, 'username':username, 'current_user_followings':current_user_followings, 'notifications':notifications, 'chats':chats})
    else:
        return HttpResponseRedirect(reverse_lazy('signin:signin'))
    ...
    if request.session.has_key('username'):
        ...
        for c in chats_list:
            if chats_count < 5:
                ...
                break
        return render(request, "followers.html", {'profiles':profiles, 'username':username, 'current_user_followers':current_user_followers,'notifications':notifications, 'chats': chats})
    else:
        return HttpResponseRedirect(reverse_lazy('signin:signin'))
