    ...
    if request.session.has_key('username'):
        ...
        for c in chats_list:
            if chats_count < 5:
                ...
                break
        return render(request, "customViewProfile.html", {'user_img':user_img, 'username':username, 'profile_user_name':profile_user_name, 'profile_user_email':profile_user_email, 'current_user_followings':current_user_followings, 'user_profile_details':user_profile_details, 'follower':nfollower,'following':nfollowing, 'notifications':notifications, 'chats': chats})
    else:
        return HttpResponseRedirect(reverse_lazy('signin:signin'))
